...
Nesse projeto, utilizei o molde MDF fornecido pelo professor em sala. O circuito foi com a mesma lógica do anterior, apenas com essas mudanças: utilizei jumpers macho-fêmea para ligar as LEDs ao Arduino e aos resistores na protoboard.

<img src="https://github.com/user-attachments/assets/bbfa29dd-f2c1-4e29-a48d-92e32d8c9a81" width="600">

Vídeo: 
<https://drive.google.com/file/d/1t87zKLTMW-4_KQzpfJmixMEiQdVbLGaG/view?usp=sharing>

Neste código, a POO (Programação Orientada a Objetos) é utilizada para definir os "moldes" (as Classes `Led` e `Semaforo`) e criar instâncias funcionais (os Objetos `ledVerde` e `meuSemaforo`), encapsulando a lógica de funcionamento de cada parte. Os Ponteiros (`*`) são a "cola" que permite ao objeto `Semaforo` gerenciar os objetos `Led` sem copiá-los; ele armazena apenas o endereço de memória de cada LED (obtido com `&`) e usa o operador de seta (`->`) para acessar os métodos (como `acender()`) desses LEDs específicos.

O código também pode ser acessado pelo arquivo `semaforo-projeto2.ino` no repositório.
